...
class FormManager {
	...
	//main method
	//--------------------------------------------------------
	public static function Run($entity_name, $datos, $obj_controller, $type = 'main'){
		...
		//self::$collector->loadPlugins($entity_name);
		...
		//si es un quick_edit, los campos no pueden ser obligatorios porque podemos no actualizar nada
		if ($type == "quick_edit"){
			//establecemos todo como no requerido
			foreach ($estructura as &$def_campo){
				$def_campo["required"] = 0;
			}
			unset($def_campo);
		}
	...
	public static function HasQuickEditForm($entity_id){
		$data = query("SELECT 1 FROM app_forms WHERE id_entity = ".quote($entity_id) . " and tipo='quick_edit'");
		if (count($data)>0){return true;}
		return false;
	}
		...
		*/
		...
		foreach($list as $item_list){
			...
			if ($tipo=='column'){
				...
				$html .= '</div>';
			}else if ($tipo=='custom' && $item_list['custom_section'] != '' ){

				$html .= '<div class="'.$item_list['width'].'" style="height:'.$item_list['height'].'px">';
				$html .= self::$obj_controller->customContent($datos['id'], $item_list['custom_section'] ) ;
				$html .= '</div>';
			}
