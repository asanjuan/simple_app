...
class Controller {
	...
	public function Run(){
		...
		if ($operation != ""){
			...
			if ($operation == "guardar"){
				try{
					...
					foreach($this->estructura  as $campo){
						...
						}else if (!isset($_POST[$campo["dbcolumn"]]) 
							&& $campo["type"] != "datetime" 
							&& $campo["type"] != "calc"
							&& empty($campo["disabled"])){
							$r[$campo["dbcolumn"]] = $campo["value"]; //valores por defecto
						}
			...
			$campos_html = FormManager::Run($this->this_controller, $datos, $this);
			
			$custom_content = $this->customContent($ite,"");
