...
class FormManager {
	...
	public static function Run($entity_name, $datos, $type = 'main'){
		...
		if (count($formdata)>0){
			...
			foreach($formdata as $form){
				$html .= FormManager::PrintForm($form,$estructura, $datos);
			}
	...
	public static function PrintForm($formdata,$estructura, $datos){
		$html = "";
		...
		foreach($areadata as $area){
			$html .= FormManager::PrintArea($area,$estructura, $datos);
		}
	...
	public static function PrintArea($areadata,$estructura, $datos){
		...
		foreach($list as $item_list){
			$html .= FormManager::PrintSection($item_list,$estructura, $datos);
		}
		$html .= "</div>";
		return $html;
	...
	public static function PrintSection($sectiondata,$estructura, $datos){
		...
		foreach($list as $item_list){
			...
			if ($tipo=='column'){
				...
				$html .= print_grid($view_id , $view_column_rel , $datos['id'],$control_name);
			}else if ($tipo=='graph'){
		...
		if ($campo["hidden"]==1){
			...
			if (empty($campo["user_control"])){
				...
				switch ($campo["type"]){
					...
					case "calc":
						if ($mostrar_calculados) $html .= '<span style="display:block;border:solid 1px #cccccc;padding:5px;font-weight: bold;background-color:#eeeeee;border-radius:5px;font-size:1.5em">'.$val.'</span>';
						break;
					case "file":
				...
				switch ($campo["user_control"]){
					...
					case "image":
						$html .= '<div><input type="file" name="'.$campo["dbcolumn"].'" /></div>';
						if (isset($datos[$campo["dbcolumn"]])) {
							$html .= '<div style="border:solid 1px #cccccc;text-align:center;background-color:white;padding:5px"><img class="image_form" src="'.$datos[$campo["dbcolumn"]].'" /></div>';
						}else{
							$html .= '<div>Max '.ini_get('upload_max_filesize') .'</div>';
						}
						break;
