...
class Form_Subgrid extends RestApi {
	...
	public function loadPermissions($entity_id){
	...
	public function execute_select($data){
		...
		$this->loadPermissions($obj['id_entity']);
	...
	public function list_controller($sql, $filter,$search_text, $orderby, $page, $items_per_page, $up_down_field,$imagefield){
		...
		if ($this->access_insert) $botones .=  '<a class="boton-enlace btn-new" href="#" data-operation="new"><i class="fas fa-plus"></i> <span> Nuevo </span></a> ';		
			...
			foreach($estructura  as $campo){
				if ( $campo["type"] == 'file' ){
					...
					if (isset($_FILES[$file_id ]) && $_FILES[$file_id]["name"]!= ""){
						...
						$record[$campo["dbcolumn"]] = "/entity_files/".$controller."/".basename($_FILES[$file_id]["name"]);
